...
public class ShoppingList<T> {
    ...
    public void delete(String productName) {
        Product currentProduct = first;
        if (first.getName().equalsIgnoreCase(productName)) {
            first = first.getNext();
        } else {
            while (currentProduct.getNext() != null) {
                Product nextProduct = currentProduct.getNext();
                if (nextProduct.getName().equalsIgnoreCase(productName)) {
    ...
    public void editProduct(String productName, String name, int price, Type type) {
        ...
        if (first.getName().equalsIgnoreCase(productName)) {
            ...
            while (currentProduct.getNext() != null) {
                if (currentProduct.getNext().getName().equalsIgnoreCase(productName)) {
                    currentProduct.getNext().setName(name);
                    currentProduct.getNext().setPrice(price);
                    currentProduct.getNext().setType(type);
                    break;
                }
                currentProduct = currentProduct.getNext();
            }
